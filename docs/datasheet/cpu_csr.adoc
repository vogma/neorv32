...
5+^| **<<_table_jump_csrs>>**
| 0x017 | <<_jvt>>    | `CSR_JVT`    | URW | Table jump base address
...
| 0xc83 .. 0xc9f | <<_hpmcounterh, `hpmcounter3h`>> .. <<_hpmcounterh, `hpmcounter31h`>>     | `CSR_HPMCOUNTER3H` .. `CSR_HPMCOUNTER31H`   | URO | Performance-monitoring counter high word
...
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_zfinx_isa_extension,`Zfinx`>>
...
These CSRs are available if the RISC-V <<_zcmt_isa_extension,`Zcmt`>> ISA extension is enabled via the
top's `RISCV_ISA_Zcmt` generic. Any access when the extension is disabled raises an illegal instruction
exception.
...
[grid="none"]
|=======================
| Name        | Table jump base address
| Address     | `0x017`
| Reset value | `0x00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_zcmt_isa_extension,`Zcmt`>>
| Description | Base address of the 256-entry `cm.jt`/`cm.jalt` jump table. The table must be 64-byte
aligned: bits 5:0 (the `mode` field) are hardwired to zero (WARL). The CSR is accessible from user mode.
|=======================
...
|=======================
| Bit | R/W | Function
| 31:6 | r/w | `base` - jump table base address (64-byte aligned)
| 5:0  | r/- | `mode` - hardwired to zero
|=======================
...
| 12:11 | `CSR_MSTATUS_MPP_H` : `CSR_MSTATUS_MPP_L` | r/w | **MPP**: Previous machine privilege mode, `11` = machine-mode "M", `00` = user-mode "U"; other values will fall-back to user-mode (if enabled)
...
| Address     | `0x306`
| Reset value | `0x00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_u_isa_extension,`U`>>
...
| Reset value | `0x00000000_00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_zicntr_isa_extension,`Zicntr`>>
| Description | The `time[h]` CSRs are user-mode shadow copies the memory-mapped machine timer ("MTIMER)
of the <<_core_local_interruptor_clint>>. These CSR are read-only. Any write access will raise an illegal instruction exception.
|=======================
...
| Address     | `0xb03` ... `0xb1f` (`mhpmcounter3` ... `mhpmcounter31`)
...
[grid="none"]
|=======================
| Name        | Hardware performance monitor (HPM) counter
| Address     | `0xc03` ... `0xc1f` (`hpmcounter3` ... `hpmcounter31`)
|             | `0xc83` ... `0xc9f` (`hpmcounter3h` ... `hpmcounter31h`)
| Reset value | all `0x00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_zihpm_isa_extension,`Zihpm`>>
| Description | The `hpmcounter[h]` CSRs are user-mode shadow copies of the according <<_mhpmcounterh>> CSRs. The user-mode
counters are read-only. Any write access will raise an illegal instruction exception.
|=======================
...
|             | `0x721` (`mcyclecfgh`)
| Reset value | all `0x00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_zicntr_isa_extension,`Zicntr`>> & <<_smcntrpmf_isa_extension,`Smcntrpmf`>>
| Description | Halt cycle counter when CPU operates in a specific privilege mode.
Note that `mcyclecfg` is hardwired to all-zero.
|=======================
...
| Reset value | all `0x00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>> & <<_zicntr_isa_extension,`Zicntr`>> & <<_smcntrpmf_isa_extension,`Smcntrpmf`>>
| Description | Halt instruction-retired counter when CPU operates in a specific privilege mode.
Note that `minstretcfg` is hardwired to all-zero.
|=======================
...
| Reset value | `0x00000000`
| ISA         | <<_zicsr_isa_extension,`Zicsr`>>
| Description | This register is hardwired to the value provided by the `OCD_JEDEC_ID` <<_processor_top_entity_generics>>
...
| 31:7 | r/- | JEDEC ID bank
| 6:0  | r/- | JEDEC ID offset
|=======================
...
|    3 | `CSR_MXISAH_ZCMT`  | r/- | <<_zcmt_isa_extension>> available
|    2 | `CSR_MXISAH_ZCMP`  | r/- | <<_zcmp_isa_extension>> available
|    1 | `CSR_MXISAH_ZCMOP` | r/- | <<_zcmop_isa_extension>> available
|    0 | `CSR_MXISAH_ZBC`   | r/- | <<_zbc_isa_extension>> available
|=======================
